...
class EnhancedEarningsScanner:
    ...
    def _is_likely_optionable(self, symbol: str) -> bool:
        """Check if a stock is likely optionable based on yfinance data"""
        try:
            stock = yf.Ticker(symbol)
            options_chain = stock.options
            return bool(options_chain)
        except Exception:
            return False
            ...
            for line in lines[1:]:
                try:
                    ...
                    if symbol and earnings_date_str:
                        ...
                        if 0 <= days_to_earnings <= days_ahead and self._is_likely_optionable(symbol):
