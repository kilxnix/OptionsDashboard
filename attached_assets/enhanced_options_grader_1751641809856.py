...
def example_usage():
    """
    Example showing how the grader would score GOOGL and COST options
    """
    grader = EnhancedOptionsGrader(os.getenv('ALPHA_VANTAGE_API_KEY'))
    
    # GOOGL $200 Call example
    googl_option = {
        'symbol': 'GOOGL',
        'strike': 200,
        'type': 'call',
    ...
    # COST $1040 Call example  
    cost_option = {
        'symbol': 'COST',
        'strike': 1040,
        'type': 'call',
        'expiration': '2025-01-17',  # Guessing expiration
        'bid': 4.05,
        'ask': 6.00,
        'mark': 5.03,
        'volume': 0,
        ...
        'theta': -0.2607,
        'vega': 0.7155,
        'implied_volatility': 0.1978
    }
    
    # Mock market data
    market_data = {
        'current_price': 179.16,  # GOOGL
        'volatility_30d': 35,
        'sector': 'Technology'
    }
    
    print("GOOGL $200 Call Analysis:")
    googl_score, googl_analysis = grader.calculate_option_score(googl_option, market_data)
    print(f"Total Score: {googl_score:.1f}/100")
    print(f"Recommendation: {googl_analysis['recommendation']}")
    print(f"Confidence: {googl_analysis['confidence']}%")
    print(f"Hold Period: {googl_analysis['holding_period']['recommended_days']} days")
    print(f"Components: {googl_analysis['components']}")
    
    print("\n" + "="*50 + "\n")
    
    # Update market data for COST
    market_data['current_price'] = 981.80
    market_data['sector'] = 'Consumer Defensive'
    
    print("COST $1040 Call Analysis:")
    cost_score, cost_analysis = grader.calculate_option_score(cost_option, market_data)
    print(f"Total Score: {cost_score:.1f}/100")
    print(f"Recommendation: {cost_analysis['recommendation']}")
    print(f"Confidence: {cost_analysis['confidence']}%")
    print(f"Components: {cost_analysis['components']}")
