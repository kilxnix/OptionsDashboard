...
class ExplosiveOptionsScanner:
    ...
    def __init__(self, alpha_vantage_key: str, base_dir: str = "./TradingPlans"):
        ...
        self.monitor = AdaptiveMarketMonitor(alpha_vantage_key)
    ...
    def run_explosive_scan(self, 
        ...
        regime = self.monitor.update_market_regime()
        print(f"📊 Market Regime: {regime['current_regime']}")
        ...
        # Apply pre-filters based on regime
        symbols = self._apply_regime_filters(symbols, regime)
    ...
    def monitor_active_positions(self) -> Dict:
        ...
        for position_id, position_data in active_positions.items():
            ...
            # Monitor position
            result = self.monitor.monitor_position(position_id, position_data)
            
            monitoring_results['positions_monitored'] += 1
            
            # Aggregate alerts
            if result['alerts']:
                monitoring_results['alerts'].extend([
            ...
            if result['exit_signals']:
                monitoring_results['exit_signals'].extend([
                    {**signal, 'position_id': position_id}
                    for signal in result['exit_signals']
                ])
                print(f"  🚨 EXIT SIGNAL: {result['exit_signals'][0]['action']}")
            
            # Aggregate adjustments
            if result['adjustments']:
                monitoring_results['adjustments'].extend([
                    {**adj, 'position_id': position_id}
    ...
    def _discover_symbols(self, scan_type: str) -> List[str]:
        ...
        if scan_type == 'earnings':
            ...
            best_performers = self.tracker.get_best_performing_symbols()[:20]
            
            # Combine and dedupe
            all_symbols = list(set(earnings + movers + unusual + best_performers))
            symbols = all_symbols
    ...
    def _apply_regime_filters(self, symbols: List[str], regime: Dict) -> List[str]:
        """Apply filters based on market regime"""
        # In high volatility, focus on liquid names
        if regime['current_regime']['volatility'] in ['high', 'extreme']:
            # Filter for high liquidity stocks
            liquid_symbols = ['SPY', 'QQQ', 'AAPL', 'MSFT', 'GOOGL', 'AMZN', 
                            'TSLA', 'NVDA', 'META', 'JPM', 'BAC', 'XLF']
            symbols = [s for s in symbols if s in liquid_symbols] + symbols[:30]
        
        return list(dict.fromkeys(symbols))[:75]  # Dedupe and limit
...
Market Regime: {results['scan_metadata']['market_regime']}
...
"""
    ...
    def _adapt_from_history(self):
        ...
        try:
            ...
            if perf_data:
                ...
                # Update focus list with best performers
                best_symbols = self.tracker.get_best_performing_symbols(min_trades=3)
                if best_symbols:
                    self.scan_config['focus_list'] = best_symbols[:10]
                    print(f"📈 Adapted focus list: {self.scan_config['focus_list']}")
    ...
    def _generate_summary_actions(self, monitoring_results: Dict) -> List[str]:
        ...
        # Profit targets hit
        profit_exits = [s for s in monitoring_results['exit_signals']
                       if 'PROFIT_TARGET' in s.get('type', '')]
        if profit_exits:
            actions.append(f"💰 PROFITS: {len(profit_exits)} positions hit profit targets")
        
        # Adjustments needed
        if monitoring_results['adjustments']:
            actions.append(f"🔧 ADJUSTMENTS: {len(monitoring_results['adjustments'])} positions need adjustments")
...
def run_explosive_scanner_example():
    """Example of running the explosive scanner"""
    
    # Initialize scanner
    scanner = ExplosiveOptionsScanner(os.getenv('ALPHA_VANTAGE_API_KEY'))
    
    # Run comprehensive scan
    print("🚀 Running Explosive Options Scanner...\n")
    ...
    for i, pick in enumerate(results['top_picks'][:5], 1):
        print(f"\n{i}. {pick['symbol']} - Score: {pick['score']:.1f}")
        print(pick['formatted_plan'])
        print("="*80)
    
    # Monitor positions (if any active)
    print("\n📡 Monitoring Active Positions...")
    monitoring = scanner.monitor_active_positions()
    
    if monitoring['summary_actions']:
        print("\n⚡ REQUIRED ACTIONS:")
        for action in monitoring['summary_actions']:
            print(f"  • {action}")
