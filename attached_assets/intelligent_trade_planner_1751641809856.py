...
class IntelligentTradePlanner:
    ...
    def generate_intelligent_plan(self, 
        ...
        # 4. Set intelligent stop loss based on support/volatility
        stop_loss = self._calculate_stop_loss(
            option_data, market_data, targets
        )
    ...
    def _calculate_profit_targets(self, option_data: Dict, score_analysis: Dict, 
        ...
        tech_levels = self._fetch_resistance_levels(option_data['symbol'])
        ...
        for i, (target_key, multiplier) in enumerate(zip(targets.keys(), base_multipliers)):
            ...
            # Align with technical levels if available
            if tech_levels and i < len(tech_levels):
                # Calculate what stock price would make option worth the target
                stock_target = self._calculate_stock_price_for_option_target(
                    option_data, base_target, current_stock_price
                )
                
                # Find nearest resistance
                nearest_resistance = min(tech_levels, 
                                       key=lambda x: abs(x - stock_target))
                
                # Recalculate option price at resistance
                aligned_target = self._estimate_option_price_at_stock_price(
                    option_data, nearest_resistance, current_stock_price
                )
                
                targets[target_key] = {
                    'price': round(max(base_target, aligned_target), 2),
                    'percentage': round((max(base_target, aligned_target) / option_price - 1) * 100, 1),
                    'stock_price': nearest_resistance,
    ...
    def _calculate_stop_loss(self, option_data: Dict, market_data: Dict, 
                           targets: Dict) -> Dict:
        """
        ...
        # Technical stop based on support
        support_levels = self._fetch_support_levels(option_data['symbol'])
        technical_stop = option_price * 0.75  # Default 25% stop
        
        if support_levels:
            # Calculate option price if stock hits support
            current_stock = market_data.get('current_price', 100)
            nearest_support = max([s for s in support_levels if s < current_stock], 
                                default=current_stock * 0.97)
            
            support_option_price = self._estimate_option_price_at_stock_price(
                option_data, nearest_support, current_stock
            )
            technical_stop = max(support_option_price * 0.9, option_price * 0.5)
        ...
        stop_price = max(volatility_stop, technical_stop, theta_stop, option_price * 0.65)
    ...
    def _generate_exit_strategy(self, option_data: Dict, score_analysis: Dict,
        ...
        exit_rules = {
            ...
            'opportunity_exits': {
                'iv_spike': {
                    'trigger': "IV increases 20%+ intraday",
                    'action': "Consider full exit on IV expansion"
                },
                'technical_breakdown': {
                    'trigger': "Stock breaks key support",
                    'action': "Exit immediately regardless of option price"
                },
                'unusual_activity': {
                    'trigger': "Massive put buying detected",
    ...
    def _analyze_risk_reward(self, entry_price: float, targets: Dict, 
        ...
        # Add stop loss probability (assume remaining probability)
    ...
    def _generate_execution_notes(self, option_data: Dict, score_analysis: Dict,
        ...
        # Market regime notes
        if market_data.get('volatility_regime') == 'high_vol':
            notes.append("🌊 HIGH VOLATILITY REGIME: Expect larger swings, size down")
        
        # Earnings notes
        if market_data.get('earnings_info', {}).get('is_pre_earnings'):
            days_to_earnings = market_data['earnings_info'].get('days_to_earnings', 'unknown')
            notes.append(f"📅 EARNINGS PLAY: Earnings in {days_to_earnings} days, expect IV expansion")
...
"""
    ...
    def _fetch_resistance_levels(self, symbol: str) -> List[float]:
        """Calculate resistance levels using Alpha Vantage data"""
        # Would implement pivot points, recent highs, round numbers
        return []  # Placeholder
    
    def _fetch_support_levels(self, symbol: str) -> List[float]:
        """Calculate support levels using Alpha Vantage data"""
        # Would implement pivot points, recent lows, moving averages
        return []  # Placeholder
    
    def _calculate_stock_price_for_option_target(self, option_data: Dict, 
                                               target_price: float, 
                                               current_stock: float) -> float:
        """Calculate what stock price would achieve target option price"""
        # Simplified - would use Black-Scholes in production
        delta = option_data['delta']
        move_needed = (target_price - option_data['mark']) / delta
        return current_stock + move_needed
    
    def _estimate_option_price_at_stock_price(self, option_data: Dict,
                                            stock_price: float,
                                            current_stock: float) -> float:
        """Estimate option price at given stock price"""
        # Simplified - would use Black-Scholes in production
        delta = option_data['delta']
        gamma = option_data['gamma']
        move = stock_price - current_stock
        
        # Delta + gamma adjustment
        new_price = option_data['mark'] + (delta * move) + (0.5 * gamma * move * move)
...
def example_intelligent_plan():
    """
    Example showing how to generate an intelligent plan
    """
    ...
    option_data = {
        ...
        'theta': -0.0668,
        'vega': 0.0823,
        'implied_volatility': 0.3501
    }
    
    # Score analysis from grader
    score_analysis = {
        'total_score': 68.5,
        'confidence': 72,
        'risk_level': 'MEDIUM',
        'components': {
            'liquidity_score': 14,
            'greeks_score': 13,
            'unusual_activity_score': 18,
            'technical_score': 10,
            'iv_opportunity_score': 7,
    ...
    plan = planner.generate_intelligent_plan(
        option_data, score_analysis, market_data, account_size=25000
    )
    
    print(plan['formatted_text'])
    return plan


if __name__ == "__main__":
    example_intelligent_plan()
